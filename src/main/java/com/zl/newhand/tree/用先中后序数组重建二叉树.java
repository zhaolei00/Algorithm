...
public class 用先中后序数组重建二叉树 {
    ...
    public static TreeNode<Integer> buildTree1(int[] xianArr, int[] zhongArr) {
    ...
    private static TreeNode<Integer> f(int[] xianArr, int L1, int R1, int[] zhongArr, int L2, int R2, Map<Integer, Integer> map) {
        if (L1 > R1) {
            return null;
        }
        if (L1 == R1) { // 就一个节点，直接返回。
